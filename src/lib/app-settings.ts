...
export async function getPredictableStatuses(): Promise<string[]> {
  ...
  // 'anytime' and 'manual_lock' both allow in_progress
  // (manual_lock adds per-match checks separately)
...
/**
 * Check if per-match lock enforcement is active (manual_lock mode).
 */
