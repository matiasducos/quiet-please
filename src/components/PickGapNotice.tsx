...
/**
 * Everything the bar would say and do for this prompt.
 *
 * Pure, and exported so it is not the component that owns the copy. /admin/banners
 * renders this same object as a report of what is on the site right now — reading
 * it rather than restating it is what stops that page becoming a stale second
 * copy of the wording.
 */
export function pickGapNoticeSpec(prompt: PickGapPrompt): NoticeSpec {
  ...
  return {
    ...
    dismissCookieName: `qp_gap_${prompt.tournamentId}_${prompt.round}`,
...
export default async function PickGapNotice({ prompt }: { prompt: PickGapPrompt }) {
  const spec = pickGapNoticeSpec(prompt)
