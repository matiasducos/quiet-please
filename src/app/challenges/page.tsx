...
export default async function ChallengesPage() {
  ...
  // Batch-fetch live points for active challenges (single query)
  const activeChallengeIds = (rawChallenges ?? []).filter(c => c.status === 'accepted').map(c => c.id)
  let livePredMap: Record<string, { user_id: string; points_earned: number; is_fully_locked: boolean }[]> = {}

  if (activeChallengeIds.length > 0) {
    const { data: livePreds } = await admin
      .from('predictions')
      .select('challenge_id, user_id, points_earned, is_fully_locked')
      .in('challenge_id', activeChallengeIds)

    for (const p of livePreds ?? []) {
      if (!livePredMap[p.challenge_id]) livePredMap[p.challenge_id] = []
      livePredMap[p.challenge_id].push(p)
    }
  }
  ...
  const challenges = (rawChallenges ?? []).map(c => {
    ...
    const bothLocked = myPred?.is_fully_locked === true && theirPred?.is_fully_locked === true
      ...
      <div className="max-w-5xl mx-auto px-4 md:px-8 py-10">
        ...
        {closed.length > 0 && (
          <div>
            ...
            <div className="bg-white rounded-sm border overflow-hidden" style={{ borderColor: 'var(--chalk-dim)' }}>
              {closed.slice(0, PAST_LIMIT).map(c => <ChallengeCard key={c.id} c={c} />)}
            </div>
          </div>
        )}
      </div>
