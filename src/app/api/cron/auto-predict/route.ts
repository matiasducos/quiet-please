...
export async function GET(request: Request) {
  ...
  return withCronLogging('auto-predict', async () => {
    ...
    const { data: existingPredictions } = await supabase
      .from('predictions')
      .select('id, user_id, tournament_id, is_fully_locked, pick_sources')
      .in('user_id', userIds)
      .in('tournament_id', tournamentIds)
      .is('challenge_id', null)

    const predMap = new Map<string, { id: string; is_fully_locked: boolean; isAutoGenerated: boolean }>()
    for (const pred of existingPredictions ?? []) {
      // Check if ALL picks were auto-generated (not manual)
      const sources = (pred.pick_sources as Record<string, string>) ?? {}
      const allAuto = Object.keys(sources).length > 0 && Object.values(sources).every(s => s === 'auto')
      predMap.set(`${pred.user_id}:${pred.tournament_id}`, {
        id: pred.id,
        is_fully_locked: pred.is_fully_locked,
        isAutoGenerated: allAuto,
      })
    }
    ...
    for (const tournament of eligibleTournaments) {
      ...
      for (const userId of userIds) {
        try {
          ...
          // b. Check if user already has a fully-locked prediction
          const existingPred = predMap.get(`${userId}:${tournament.id}`)
          if (existingPred?.is_fully_locked && !existingPred.isAutoGenerated) {
            // Manual lock — respect it, don't overwrite
            skipped.push({ userId, reason: 'already_fully_locked_manual' })
            continue
          }
          // Auto-generated locked predictions CAN be re-generated (e.g., with match results)
