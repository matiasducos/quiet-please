    ...
    <div className="min-h-screen flex" style={{ background: 'var(--chalk)' }}>
      <div className="hidden lg:flex flex-col justify-between w-1/2 p-12" style={{ background: 'var(--court-dark)' }}>
        ...
        <div>
          ...
          {['Free to play', 'ATP & WTA tournaments', 'Private leagues & challenges'].map(f => (
        ...
        <div className="max-w-sm w-full mx-auto">
          ...
          <form onSubmit={handleSignup} className="flex flex-col gap-4">
            ...
            <label className="flex items-start gap-2.5 cursor-pointer">
              <input type="checkbox" checked={consented} onChange={e => { setConsented(e.target.checked); if (e.target.checked && error === CONSENT_REQUIRED) setError(null) }} required
            ...
            <button type="submit" disabled={loading || !consented} className="w-full py-3 text-sm font-medium text-white rounded-sm hover:opacity-90 disabled:opacity-50" style={{ background: 'var(--court)' }}>
          ...
          <div className="flex flex-col gap-3">
            <button onClick={handleGoogleSignup} disabled={!consented} className="w-full py-3 text-sm font-medium rounded-sm border hover:opacity-90 disabled:opacity-50 flex items-center justify-center gap-2.5 transition-opacity" style={{ borderColor: '#dadce0', color: '#3c4043', background: 'white' }}>
