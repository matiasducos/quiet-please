...
export default async function FriendsPage({
  ...
  const activityItems = friendIds.length > 0
    ? await getFriendActivity(user.id, 15)
    : []
          ...
          <div className="mt-2">
            ...
            {activityItems.length === 0 ? (
              ...
              <div className="bg-white rounded-sm border overflow-hidden" style={{ borderColor: 'var(--chalk-dim)' }}>
                ...
                {activityItems.length === 15 && (
                  <div className="px-5 py-3 border-t text-center" style={{ borderColor: 'var(--chalk-dim)' }}>
                    <span style={{ fontSize: '0.8rem', color: 'var(--muted)', fontFamily: 'var(--font-mono)' }}>
                      Showing last 15 events
                    </span>
                  </div>
                )}
              </div>
