...
export default async function UserPicksPage({
  ...
  let predQuery = supabase
    .from('predictions')
    .select('id, picks, is_fully_locked, is_published, points_earned')
    .eq('tournament_id', id)
  ...
  // OFFER this page. They read the same `is_published` column, so offering the
  // page and serving it cannot disagree — two hand-kept copies of this rule is
  // exactly what produced a "view picks" link that 404s.
  const isOwnPicks = viewer?.id === targetUser.id
  if (prediction && !isOwnPicks && prediction.is_published !== true) {
    notFound()
  }
