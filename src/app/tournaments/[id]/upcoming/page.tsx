...
export async function generateMetadata({ params }: { params: Promise<{ id: string }> }): Promise<Metadata> {
  ...
  return { title: tournament ? `Upcoming Matches — ${tournament.name}` : 'Upcoming Matches' }
}

export default async function TournamentUpcomingPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = await params
    ...
    <main className="min-h-screen" style={{ background: 'var(--chalk)' }}>
      ...
      <div className="max-w-3xl mx-auto px-4 md:px-8 py-10">
        ...
        <div className="flex items-center gap-2 mb-6" style={{ fontSize: '0.8rem', color: 'var(--muted)', fontFamily: 'var(--font-mono)' }}>
          ...
          <span>/</span>
          <span style={{ color: 'var(--ink)' }}>Upcoming</span>
        </div>
