...
- [ ] **Save draft picks** — Create a new prediction, make some picks, hit "Save draft". Verify picks persist on page reload.
- [ ] **Lock individual pick** — Make a pick, click the lock icon on a single match. Verify it shows "LOCKED ✓", can't be changed, and survives page reload.
- [ ] **Lock all picks** — Click "Lock all picks", confirm dialog. Verify all picks show locked, nothing editable, `is_fully_locked = true` in DB.
- [ ] **Auto-lock on match result** — Enter a match result (via admin/cron), reload predict page. Verify that match shows "PLAYED" badge and pick can't be changed.

## In-Progress Editing

- [ ] **Edit unplayed matches during in_progress** — Set tournament to `in_progress`. Verify you can still pick/change unplayed matches but played ones are frozen.
- [ ] **Downstream clearing respects locks** — Pick Player X through multiple rounds. Lock an early-round pick. Change a later pick — verify locked picks are NOT cleared.

## Scoring

- [ ] **Award points per-match scoring** — Run award-points cron after entering results. Verify points awarded only when `picks[matchId] === winner` (not if winner appears in any slot).
...
- [ ] **Practice mode fully removed** — Verify no "Practice" buttons on completed tournaments, no practice banner, no `is_practice` references in UI anywhere.
